...
import static servidor.Signatures.*;

/**
 *
 * @author tiago
 */
public class DH_KeyAgreement_Servidor {
    ...
    public DH_KeyAgreement_Servidor(byte[] selfPrivateKey, byte[] otherPublicKey) throws Exception {
    ...
    public ArrayList<SecretKey> DH_key_exchange_Server(OutputStream  out_s,InputStream  in_s) throws Exception{
        ...
        byte[] signature = Signatures.generatingSignature(this.selfPrivateKey, pubClienteServer );
        byte[] sts_pubKey = cipherAES( chaveSessao , signature);
