...
class ThreadCliente implements Runnable{
    ...
    public void run() {
        try {
            ...
            PublicKey clientPub = certificateHandler.verifyThenConvert(readFile("cacert.pem"), readFile("client_cert.pem"));
            PrivateKey serverPriv = certificateHandler.converterByteToPrivKey(readFile("server_key.pk8"));
            
            DH_KeyAgreement dh_instance = new DH_KeyAgreement(serverPriv,clientPub);
